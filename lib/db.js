  ...
  return {
    ...
    createOrganizerThenEvent : function(event, organizer) {
      ...
      .then((id) => {
        ...
        return db('events').insert(newEvent).returning('hashed_url');
      })
      .catch((error) => {
        console.log(error);
        return false;
     ...
     *  1) dates     : typeof Array   : Contain the dates (minimum 2) as separate strings
    ...
    /*
     * Returns all the event info
     *
     * @param eventID: typeof String : A string with the unique event ID
     * 
     * @returns a Promise with a parameter that IF successful: equals the rows of a table that contains
     *  information about the event, options, and responses.
     * , ELSE: equals false
     * 
     ...
     */
    getEventPageData : function(eventID) {
      return db('events')
      .leftOuterJoin('options', 'options.events_id', '=', 'events.hashed_url')
      .leftOuterJoin('responses', 'responses.events_id', '=', 'events.hashed_url')    
      .select('events.name', 'events.description', 'events.location', 'options.dates AS options_dates', 'responses.attendee_name', 'responses.dates AS responses_dates')
      .where({hashed_url: eventID})
      .then((rows) => {
        if (rows.length !== 0) {
          return rows;
     ...
     *  3) dates          : typeof Array : Contains 0 or 1 indicating true or false (minimum 2) as
     *    separate elements of the array to indicate whether or not the selected date works for a given 
     *    event
    ...
    updateResponse : function (updatedResponse) {
      return db('responses')
      .where({attendee_email: updatedResponse.attendee_email})
