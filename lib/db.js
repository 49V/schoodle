...
module.exports = function makeDatabaseHelpers(db) {
  return {
    ...
    getEventPageData : function(eventID) {
      return db('events')
      .join('responses', 'responses.events_id', '=', 'events.hashed_url')
      .join('options', 'options.events_id', '=', 'events.hashed_url')
      .select('events.description', 'events.location', 'options.dates AS options_dates', 'responses.attendee_name', 'responses.attendee_email', 'responses.dates AS responses_dates')
      .where({hashed_url: eventID})
      .then((rows) => {
        if (rows.length !== 0) {
          return rows;
